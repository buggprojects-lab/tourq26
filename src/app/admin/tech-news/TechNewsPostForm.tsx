...
function readLocalDraft(key: string): Partial<TechNewsPost> | null {
  if (typeof window === "undefined") return null;
  try {
    const raw = localStorage.getItem(key);
    return raw ? (JSON.parse(raw) as Partial<TechNewsPost>) : null;
  } catch {
    return null;
  }
}

export function TechNewsPostForm({ post, siteUrl, siteName }: Props) {
  const router = useRouter();
  const isNew = !post;
  const draftKey = `torq.admin.tech-news.draft.${post?.slug ?? "new"}`;
  ...
  const [dek, setDek] = useState(post?.dek ?? savedDraft?.dek ?? "");
  const [category, setCategory] = useState<TechNewsCategory>(
    post?.category ?? savedDraft?.category ?? "AI & ML",
  );
  ...
  const [status, setStatus] = useState<TechNewsStatus>(post?.status ?? "draft");
  ...
  const submit = useCallback(
    async (nextStatus: TechNewsStatus) => {
      setError("");
      setSuccessMsg("");
      setSaving(true);
      setPendingAction(isNew ? (nextStatus === "draft" ? "draft" : "publish") : "save");
      const url = post
        ? `/api/admin/content/tech-news/${encodeURIComponent(post.slug)}`
        : "/api/admin/content/tech-news";
      ...
      if (isNew && data?.slug) {
        router.push(`/admin/tech-news/${encodeURIComponent(data.slug)}/edit`);
        router.refresh();
      } else {
        setSuccessMsg(
      ...
      <div className="sticky top-0 z-20 -mx-5 mb-6 flex flex-wrap items-center justify-between gap-3 border-b border-border/60 bg-background/95 px-5 py-3 backdrop-blur lg:-mx-8 lg:px-8">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <p className="hidden truncate text-[14px] font-medium text-foreground sm:block">
            {title || (isNew ? "New story" : post?.title)}
          </p>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_340px]">
            ...
            <div className="space-y-5">
              ...
              >
                <input
                  ...
                  placeholder="Write a headline that names what changed…"
                />
              </Field>

              <Field
                label="Dek"
                hint="Short subtitle shown between the headline and the excerpt."
                charCount={dek.length}
                softMax={140}
              >
                <textarea
                  value={dek}
                ...
                <div className="mb-2 flex justify-end">
                  <AiGenerateButton<string>
                    ...
                    context={{
                      ...
                      brief: [description, dek, focusKeyword && `Focus keyword: ${focusKeyword}`, `Category: ${category}`]
                        .filter(Boolean)
                        .join(". "),
                    }}
                    onResult={setBody}
            ...
            <div className="space-y-5">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <p className="text-sm text-muted-foreground">
                  Live Google and social previews. Generate meta from the story body anytime.
                </p>
              ...
              <SerpPreview
                siteUrl={siteUrl}
                path={effectiveSlug ? `/tech-news/${effectiveSlug}` : "/tech-news"}
                title={(seoTitle.trim() || title) + (siteName ? ` | ${siteName}` : "")}
                description={description}
              />
              <SocialPreview
                siteUrl={siteUrl}
                path={`/tech-news/${effectiveSlug || "your-slug"}`}
        ...
        <aside className="space-y-5">
          ...
          <Card title="URL">
            ...
            {effectiveSlug ? (
              <p className="mono-label mt-2 truncate text-muted-foreground">
                /TECH-NEWS/{effectiveSlug.toUpperCase()}
              </p>
            ) : null}
          </Card>
          ...
          <Card title="CATEGORY">
            <select
              value={category}
              onChange={(e) => setCategory(e.target.value as TechNewsCategory)}
              className="text-input text-[13px]"
            >
              {TECH_NEWS_CATEGORIES.map((c) => (
          ...
          <Card title="SEO">
            ...
            <SidebarField label="FOCUS KEYWORD">
              <input
                ...
                placeholder="e.g. post-quantum tls"
                className="text-input text-[13px]"
              />
              <p className="mono-label mt-1 text-muted-foreground">
...
function StatusPill({ status }: { status: TechNewsStatus }) {
